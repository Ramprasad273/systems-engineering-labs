...
1. **Ingestion:** A raw syslog line is generated by an HDFS data node:
   `081109 203615 148 E block* NameSystem.allocateBlock: /user/root/data/file.txt. _blk_-1608999687919862906`
2. **Pre-Masking:** `LogTokenizer.preprocess()` strips dynamic variables:
   `<DATE> <TIME> 148 E block* NameSystem.allocateBlock: /user/root/data/file.txt. <HEX>`
3. **Tokenization:** BPE encodes the clean string into a tensor of integer IDs of length $N=18$:
